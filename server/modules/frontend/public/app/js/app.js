    ...
    angular
        ...
        .constant('APP_REQUIRES', {
          ...
          modules: [
			...
			{name: 'socket.io',                 files: ['vendor/socket.io/dist/socket.io.js']},
...
(function() {
    ...
    function routesConfig($stateProvider, $locationProvider, $urlRouterProvider, helper){
		...
		.state('app.engine-settings', {
              url: '/engine-settings',
              title: 'Engine Settings',
              templateUrl: helper.basepath('engine-settings.html')
          })
		  .state('app.engine-monitor', {
              url: '/engine-monitor',
              title: 'Engine Monitor',
			  resolve: helper.resolveFor('ngTable', 'ngTableExport', 'socket.io'),
              templateUrl: helper.basepath('engine-monitor.html'),
			  controller: 'Engine as engine'
          })
		  .state('app.engine-integrations', {
              url: '/engine-integrations',
              title: 'Engine Monitor',
			  resolve: helper.resolveFor('ngTable', 'ngTableExport', 'socket.io'),
              templateUrl: helper.basepath('engine-integrations.html'),
			  controller: 'Integrations as integrations'
          })
		  .state('app.integrations-settings', {
              url: '/integrations-settings',
              title: 'Integrations Settings',
              templateUrl: helper.basepath('integrations-settings.html')
          })
		   .state('app.api-management', {
              url: '/api-management',
              title: 'API Management',
              //templateUrl: helper.basepath('api-management.html'),
			  controller: 'Management'
          })
		  .state('app.docs', {
              url: '/documentation',
              title: 'Documentation',
              templateUrl: helper.basepath('engine-documentation.html'),
			  resolve: helper.resolveFor('flatdoc')
          })
          // Forum
          // -----------------------------------
		  ...
		  .state('app.events', {
              url: '/events',
              title: 'Events',
              templateUrl: helper.basepath('table-events.html'),
              resolve: helper.resolveFor('ngTable', 'ngTableExport', 'socket.io'),
			  controller: 'Events as log'
          })
		  ...
		  .state('page.underdev', {
              ...
              templateUrl: 'app/pages/underdev.html'
          })
          //
          // Horizontal layout
          // -----------------------------------
...
(function() {
    ...
    function settingsRun($rootScope, $localStorage){
      ...
      $rootScope.user = {
        name:     'João Caldeira',
        job:      'Principal Solution Engineer',
		email:    'joao_caldeira@bmc.com',
      ...
      // -----------------------------------
      $rootScope.app = {
        name: 'BMC TrueSight Integration Framework',
        description: '',
...
/*
	...
	*/
    ...
    angular
		...
		.controller('Integrations', Integrations)
		.controller('Management', Management)

	//Events.$inject = ['$log'];
	Management.$inject = ['$window','$location'];
	EngineService.$inject = ['$resource'];
	Events.$inject = ['$log','$filter', 'ngTableParams', '$scope'];
	Engine.$inject = ['$log','$filter', 'ngTableParams', '$scope', 'EngineService'];
	Integrations.$inject = ['$log','$filter', 'ngTableParams', '$scope','EngineService'];
	
	function Management($window,$location) {
		...
		$location.path("/app/engine-integrations");
		$window.open('/app/views/api-management');
		
		}
		
	}
		
	function EngineService($resource) {
        // Loads the list of files to populate the treeview
        var vm = this;
		     	
		vm.Integrations = Integrations;
		vm.EngineComponents = EngineComponents;
		vm.Commands = Commands;
		
		function EngineComponents() {
									
				return $resource( '/api/engine/status', { }, 
					{
		...
		function Integrations() {
				...
				return $resource( '/api/engine/integrations', { }, 
					{
						//query: { method: 'JSONP', params: { callback: 'JSON_CALLBACK' } }
						query:  { method:'GET', params:{}, isArray:true },
						start:  { method:'GET', params:{}, isArray:true },
						stop:  { method:'GET', params:{}, isArray:true }
						//update: { method:'POST', params:{ } }
					
				});
			}
			
		function Commands(integration, action) {
				
				console.log("Executing : " + integration + ' ' + action);
				
				return $resource( '/api/' + integration + '/collection/' + action , { }, 
					{
						execute:  { method:'GET', params:{}, isArray:false }					
				});
			}
	...
	function Integrations($log, $filter, ngTableParams, $scope, EngineService) {
        ...
        $scope.refresh = function() {
		  ...
		  EngineService.Integrations().query();
					  
	    };
	   
	    $scope.start = function(index) {
		 
		  var integration = $scope.integrations[index].name.toLowerCase();	 
		  EngineService.Commands(integration,'start').execute();
		  $scope.refresh();
		...
		$scope.stop = function(index) {
		  ...
		  var integration = $scope.integrations[index].name.toLowerCase();
		  EngineService.Commands(integration,'stop').execute();
		  $scope.refresh();
        ...
        socket.on('integrations', function(msg){
	       
		  $scope.integrations = msg;
	...
	function Engine($log, $filter, ngTableParams, $scope, EngineService) {
        // for controllerAs syntax
        var vm = this;
		...
		$scope.refresh = function() {
		  ...
		  console.log("Getting Engine Status");
		  EngineService.EngineComponents().query();
    ...
    function Events($log, $filter, ngTableParams, $scope) {
        // for controllerAs syntax
        var vm = this;
		...
		//$log.log('From the Events controller');		  
        ...
        socket.on('message', function(msg){
		  ...
		  vm.tableParams = new ngTableParams({
              ...
              getData: function($defer, params) {
                  // use build-in angular filter
                  var orderedData = params.sorting() ?
                          $filter('orderBy')(events, params.orderBy()) :
                          events;
